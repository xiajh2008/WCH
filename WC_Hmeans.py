...
def user_defined_Kmeans(data, hubs_vector):
    from pyclustering.cluster.kmeans import kmeans, kmeans_visualizer
    # from pyclustering.cluster.center_initializer import kmeans_plusplus_initializer

    """# Prepare initial centers using K-Means++ method."""
    # initial_centers = kmeans_plusplus_initializer(data, 9).initialize()

    """# Create instance of K-Means algorithm with prepared centers."""
    # kmeans_instance = kmeans(data, initial_centers)
    kmeans_instance = kmeans(data, hubs_vector)

    """# Run cluster analysis and obtain results."""
    kmeans_instance.process()
    clusters_kmeans = kmeans_instance.get_clusters()
    final_centers_kmeans = kmeans_instance.get_centers()

    """# Visualize obtained results"""
    # kmeans_visualizer.show_clusters(data, clusters_kmeans, hubs_vector)

    """自定义类标签"""
    label_kmeans = [-1 for x in range(data.shape[0])]
    for i in range(len(clusters_kmeans)):
        for node in clusters_kmeans[i]:
            label_kmeans[node] = i

    return label_kmeans, clusters_kmeans, final_centers_kmeans
...
def clustering(data, t, e, lamda):
    ...
    label_kmeans, clusters_kmeans, final_centers_kmeans = user_defined_Kmeans(data, hubs_vector)

    """如果e==0，不计算每个潜在类的连通分量"""
    if e > 0:
        cluster_nn, final_centers_kmeans_nn = connectivity_component(data, nn_t, clusters_kmeans, final_centers_kmeans)
    else:
        cluster_nn = clusters_kmeans
        final_centers_kmeans_nn = final_centers_kmeans
